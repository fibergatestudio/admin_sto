              ...
              <div class="tabs-menu-child" id="txt_3">
                ...
                <div class="form-row">
                  ...
                  <div class="form-group col-md-6">
                    ...
                    <input type="text" name="income_to_text" class="form-control" value="" disabled>
                    <input type="hidden" name="income_to" value="">
                  </div>
                </div>
...
<script type="text/javascript">
  ...
  // Передача данных о сумме с учетом курса при "Перевод на счет"
  $('#txt_3 input[name="expense"]').bind('keyup change blur',function(){
    let sum = 0;
    let currencyTo = $('select[name="to_account_id"] option:selected').attr('data-currency');
    let expense = $('#txt_3 input[name="expense"]').val();
    //console.log(currencyTo);
    if (currencyTo !== currencyFrom) {
      if (currencyTo === 'MDL' && currencyFrom === 'USD') {
        sum = Math.round(expense/usd);
      }
      else if (currencyTo === 'MDL' && currencyFrom === 'EUR') {
        sum = Math.round(expense/eur);
      }
        ...
        let temp = Math.round(expense/eur*100)/100;
        sum = Math.round(temp*usd);
      }
      else if (currencyTo === 'EUR' && currencyFrom === 'USD') {
        let temp = Math.round(expense/usd*100)/100;
    ...
    $('#txt_3 input[name="income_to"]').val(sum);
    $('#txt_3 input[name="income_to_text"]').val(sum);
  });
  ...
  $('#createOperation').click(function(event){
    ...
    $('.alert-warning.category-message').remove();       
    ...
    if (!$('[name="category"]').val() && $('[name="to_account_id"]').prop('disabled') === true) {
      $('[name="category"]').parent().parent().before('<p class="alert-warning category-message">Выберите или введите новую категорию !</p>');
      return 0;
    }
