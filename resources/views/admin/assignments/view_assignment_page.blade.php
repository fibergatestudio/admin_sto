...
<?php //echo '<pre>'.print_r($work_directions,true).'</pre>'; ?>
          ...
          <tbody id="dynamic-{{ substr($new_sub_assignment->spares_row_index, 0, 1) }}-2">
              ...
              <tr style="background-color: {{ ($new_sub_assignment->spares_is_locked == 'active') ? '#4dffff' : 'transparent' }} ;" data-tr-row="{{ $new_sub_assignment->spares_row_index }}">
                  ...
                  <td>
                      ...
                      <button data-row="{{ $new_sub_assignment->spares_row_index }}" type="button" onclick="buttonUnlocked(this)"><i class="icon-unlocked2" style="display: {{ ($new_sub_assignment->spares_is_locked == 'active') ? 'none' : 'block' }} ;"></i></button>
...
<script type="text/javascript">
  ...
  function calculationProfitability() {
    ...
    if ($('#table-calculation-profitability').css('display') == 'block') alert("Рентабельность рассчитана, таблица находится внизу страницы !");
  }
